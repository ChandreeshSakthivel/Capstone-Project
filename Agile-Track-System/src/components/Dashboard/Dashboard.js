...
const Dashboard = () => {
    ...
    useEffect(() => {
        const fetchScrums = async () => {
            try {
                const response = await axios.get('http://localhost:5000/scrums');
    ...
    const handleGetDetails = async (scrumId) => {
        try {
            const response = await axios.get(`http://localhost:5000/scrums/${scrumId}`);
    ...
    const handleAddScrum = async (event) => {
        ...
        try {
            //Add new Scums
            const newScrumResponse = await axios.post('http://localhost:5000/scrums', {
                name: newScrumName,
            });
            ...
            const updatedScrums = await axios.get('http://localhost:5000/scrums');
